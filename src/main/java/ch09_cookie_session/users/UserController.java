	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "register":
			...
			if (pwd.equals(pwd2)) {
				...
				response.sendRedirect("/jw/ch09/users/list");	// send는 앞에 jw 붙이기
			} else {
//				out.print("<script>");
//				out.print("alert('패스워드 입력이 잘목되었습니다.');");
//				out.print("location.href='/jw/ch09/users/register.html';");
//				out.print("</script>");
				// 위 코드를 jsp로 바꿈 이부분을 많이 사용할 것 같아서
		...
		case "login":
			...
			if (result == UserService.CORRECT_LOGIN) {
				...
				rd = request.getRequestDispatcher("/ch09/users/alertMsg.jsp");
				rd.forward(request, response);
			} else {	// result == UserService.UID_NOT_EXIST

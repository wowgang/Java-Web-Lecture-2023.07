...
public class ListViewer extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (sessionUid != null) {	// 로그인 상태
			html += "<button onclick=\"location.href='/jw/ch09/users/logout'\">로그아웃</button> &nbsp;&nbsp;"; //로그아웃은 서블릿에있는 액션이니까 logout.html이 아님  / &nbsp; 6개공백
		...
		html += "    <hr>"
				+ "    <table border=\"1\">"
				+ "        <tr>"
				+ "            <th>UID</th><th>PWD</th><th>Name</th><th>email</th><th>등록일</th><th>액션</th>";
		for (User u: list) {
			html += "<tr>"
					+ "    <td>" + u.getUid() + "</td>"
